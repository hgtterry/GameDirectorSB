...
void SB_Player::Initialize()
{
	...
	//pBase->Player_Ent = App->CLSB_Ogre->mSceneMgr->createEntity("Player_1", "axes.mesh", App->CL_Ogre->App_Resource_Group);
...
void SB_Player::Check_Collisions(void)
{
	...
	int numManifolds = App->CLSB_Bullet->dynamicsWorld->getDispatcher()->getNumManifolds();
	for (int i = 0; i<numManifolds; i++)
	{
		btPersistentManifold* contactManifold = App->CLSB_Bullet->dynamicsWorld->getDispatcher()->getManifoldByIndexInternal(i);
