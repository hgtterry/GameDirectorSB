...
#pragma warning (disable:4725 4731)  // FDIV warning...whoopee!!

static void	Render_LineZBuffer(int xa, int ya, geFloat za,
							   int xb, int yb, geFloat zb,
	Ogre::uint16 color, ViewVars *Cam);
...
{
	Ogre::int32		NumPoints;
// changed QD 11/03
// cylinders, cones ... can have faces with up to 64 points
// 64 doesn't seem to be enough, the array must be exceeded somwhere!
...
static Ogre::uint32		NumASpans, RemainingCount;
static Ogre::uint32		DeltaU, DeltaV, DeltaW;
static Ogre::uint32		UFixed, VFixed, WLeft, WRight;
static Ogre::uint8		*pTex;
static Ogre::uint32		*pZBuf;						//32 bit zbuffer
...
int ClipToPlane(RenderFace *pin, GPlane *pplane, RenderFace *pout)
{
...
// end change
	...
	if(pout->NumPoints > 64 || pout->NumPoints < 3) //>=32 // changed QD 11/03
		return 0;
	else
		return 1;
}
...
void Render_ResizeView (ViewVars *v, long vx, long vy)
{
	...
	if(vx && vy)
	{
		...
		v->pZBuffer	=(Ogre::uint32 *)	geRam_Allocate(sizeof(Ogre::uint32) * (vx*vy));
	}

	v->FieldOfView		=2.0f;	//fixed for now?
	v->XScreenScale		=((geFloat)vx) / v->FieldOfView;
	v->YScreenScale		=((geFloat)vy) / v->FieldOfView;
	v->MaxScale			= std::max(v->XScreenScale, v->YScreenScale);
...
static void Render_RenderOneBrushFaceZBuffer (ViewVars *Cam, RenderFace *pFace, Ogre::uint32 LineColor)
{
	int j;
	RenderFace Clipped;
	...
	for(j=0;j < pFace->NumPoints;j++)
	{
		...
		Render_LineZBuffer( Units_Round(Clipped.Points[j].X),
							...
							Clipped.Points[k].Z,
							((Ogre::uint16)(((LineColor & 0xf8)<<7) | ((LineColor & 0xf800)>>6) | ((LineColor & 0xf80000)>>19)))
							, Cam);
	}

}

static void Render_RenderOneBrushFace (ViewVars *Cam, RenderFace *pFace, Ogre::uint32 LineColor)
{
	int j;
	RenderFace Clipped;
...
void Render_RenderBrushFaces(ViewVars *Cam, Brush *b, Ogre::uint32 LineColor)
{
	int	i;
	RenderFace	TempFace;
...
void Render_RenderBrushSelFaces(ViewVars *Cam, Brush *b, Ogre::uint32 LineColor)
{
	int	i;
	RenderFace	TempFace;
...
void Render_RenderBrushFacesZBuffer(ViewVars *Cam, Brush *b, Ogre::uint32 LineColor)
{
	int			i;
	RenderFace	TempFace;
...
//	Height	=Bitmap_GetHeight(bmap);
	...
	zval	=(Ogre::uint32)(cpos.Z * FixedScale);
	...
	src		=((Ogre::uint16 *)geBitmap_GetBits(LockedBitmap)) + (Height -1) * Width;
...
//	geBitmap_Palette_GetData (bpal, WorkingPalette, GE_PIXELFORMAT_16BIT_555_BGR, 256);
	...
	if(*(zbuf + (((sy+((ey-sy)/2)))*Cam->Width + sx+((ex-sx)/2))) > zval)
	{
		...
		{
			dst		=((Ogre::uint16 *)Cam->pBits) + (y * Cam->Width) + sx;
			zbuf	=(Ogre::uint32 *)(Cam->pZBuffer + (y * Cam->Width) + sx);
...
void	Render_3DTextureZBufferOutline(ViewVars *Cam, const geVec3d *pos, const geBitmap *bmap, Ogre::uint32 OutlineColor)
{
	int		x, y, i, Width, Height;
	int		xroll, yroll;
	int		sx, sy, ex, ey;
...
*/						*dst	=((Ogre::uint16)(((OutlineColor & 0xf8)<<7) | ((OutlineColor & 0xf800)>>6) | ((OutlineColor & 0xf80000)>>19)));
						*zbuf	=zval;
	...
	Ogre::uint16 color, ViewVars *Cam)
{
	...
	if(dy==0)
	{
		...
		if(xb < xa)
		{
			dz	=(Ogre::uint32)(((za - zb)/((geFloat)(xa-xb))) * FixedScale);
			zai	=(Ogre::uint32)(zb * FixedScale);
			tz	=(Ogre::uint32 *)(Cam->pZBuffer + (ya * Cam->Width) + xb);
			td	=((Ogre::uint16 *)Cam->pBits) + (ya * Cam->Width) + xb;
			for(;xb <= xa;xb++)
			{
				if(*tz > zai)
				{
		...
		{
			dz	=(Ogre::uint32)(((zb - za)/((geFloat)(xb-xa))) * FixedScale);
			zai	=(Ogre::uint32)(za * FixedScale);
			tz	=(Ogre::uint32 *)(Cam->pZBuffer + (ya * Cam->Width) + xa);
			td	=((Ogre::uint16 *)Cam->pBits) + (ya * Cam->Width) + xa;
			for(;xa <= xb;xa++)
			{
				if(*tz > zai)
				{
	...
	if(dx==0)
	{
		...
		if (yb < ya)
		{
			dz	=(Ogre::uint32)(((za - zb)/((geFloat)(ya-yb))) * FixedScale);
			zai	=(Ogre::uint32)(zb * FixedScale);
			tz	=(Ogre::uint32 *)(Cam->pZBuffer + (yb * Cam->Width) + xb);
			td	=((Ogre::uint16 *)Cam->pBits) + (yb * Cam->Width) + xb;
			for(;yb <= ya;yb++)
			{
				if(*tz > zai)
				{
		...
		{
			dz	=(Ogre::uint32)(((zb - za)/((geFloat)(yb-ya))) * FixedScale);
			zai	=(Ogre::uint32)(za * FixedScale);
			tz	=(Ogre::uint32 *)(Cam->pZBuffer + (ya * Cam->Width) + xb);
			td	=((Ogre::uint16 *)Cam->pBits) + (ya * Cam->Width) + xb;
			for(;ya <= yb;ya++)
...
{
	memset(Cam->pZBuffer, 0, (sizeof(Ogre::uint32) * (Cam->Width*Cam->Height)));
}
...
void	Render_RenderTree(ViewVars *Cam, GNode *n, HDC ViewDC, int RFlags)
{
    ClearEdgeLists(Cam);
