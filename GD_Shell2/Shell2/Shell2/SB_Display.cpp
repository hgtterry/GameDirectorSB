...
{
	strcpy(App->GDCL_Scene_Data->B_Counter[Index]->Panel_Name, "Not_Set");
	App->GDCL_Scene_Data->B_Counter[Index]->PosX = 250;
	App->GDCL_Scene_Data->B_Counter[Index]->PosY = 10;

	App->GDCL_Scene_Data->B_Counter[Index]->Deleted = 0;
	App->GDCL_Scene_Data->B_Counter[Index]->Altered = 0;
	App->GDCL_Scene_Data->B_Counter[Index]->Show_Panel_Flag = 0;
	App->GDCL_Scene_Data->B_Counter[Index]->Unique_ID = 0;

	strcpy(App->GDCL_Scene_Data->B_Counter[Index]->Text, "Score:= ");
...
void SB_Display::Add_New_Counter()
{
	...
	int Index = App->GDCL_Scene_Data->Counters_Count;

	App->GDCL_Scene_Data->B_Counter[Index] = new Base_Counter();
	Set_Counter_Defaults(Index);

	strcpy_s(B_Name, "Counter_");
	_itoa(Index, ConNum, 10);
	strcat(B_Name, ConNum);
	strcpy(App->GDCL_Scene_Data->B_Counter[Index]->Panel_Name, B_Name);
}
...
bool SB_Display::Add_Counters_From_File() // From File
{
	...
	int Counters_Count = App->GDCL_Scene_Data->Counters_Count;
	int Count = 0;

	while (Count < Counters_Count)
	{
		
		App->GDCL_Scene_Data->B_Counter[Count]->Counter = App->GDCL_Scene_Data->B_Counter[Count]->Start_Value;
...
{
	int Count = 0;
	int Total = App->GDCL_Scene_Data->Counters_Count;

	while (Count < Total)
	{
		if (App->GDCL_Scene_Data->B_Counter[Count]->Deleted == 0)
		{
			int Result = 1;
			Result = strcmp(App->GDCL_Scene_Data->B_Counter[Count]->Panel_Name, Name);
			if (Result == 0)
			{
				return Count;
			}
