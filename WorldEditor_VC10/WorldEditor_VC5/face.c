...
Face	*Face_CreateFromPlane(const GPlane *p, geFloat Radius, int DibId)
{
	geFloat	v;
...
//		if (f->Flags & FACE_TRANSPARENT) OutputFlags |= ffTranslucent;
	...
	TypeIO_WriteBlock (wf, &f->Face_Plane, sizeof (GPlane));
}
...
void	Face_GetSplitInfo(const Face *f, const GPlane *p, geFloat *dists, Guint8 *sides, Guint8 *cnt)
{
	int	i;
...
void	Face_Clip(Face *f, const GPlane *p, geFloat *dists, Guint8 *sides)
{
	geVec3d	*p1, *p2, mid;
	int		nfp, nbp, i, j;
	geFloat	dot;
...
void	Face_MostlyOnSide(const Face *f, const GPlane *p, geFloat *max, int *side)
{
	int		i;
	geFloat	d;
