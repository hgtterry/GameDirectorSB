...
bool WV_Export_Ogre::Export_AssimpToOgre(void)
{
	...
	//strcpy(App->CLSB_Model->szSelectedDir, App->CLSB_Model->Model_FolderPath);
...
bool WV_Export_Ogre::CreateDirectoryMesh(void)
{
	...
	//{
	//	strcpy(NewDirectory, "");
	//	_chdir(App->CLSB_Loader->szSelectedDir);
	//}
	//else
	//{
	//	strcpy(NewDirectory, "\\");
	...
	//	
	//	strcat(App->CLSB_Loader->szSelectedDir, NewDirectory);

	//	if (_mkdir(App->CLSB_Loader->szSelectedDir) == 0)
	//	{
	//		strcpy(mDecompileFolder, App->CLSB_Loader->szSelectedDir);
	//		_chdir(App->CLSB_Loader->szSelectedDir);
	//	}
	//	else
	//	{
...
bool WV_Export_Ogre::DecompileTextures(void)
{
	...
	while (Loop < MatCount)
	{
		...
		if (_stricmp(FileName + strlen(FileName) - 4, ".tga") == 0)
		{
			...
			WriteTGA(mDirectory, App->CLSB_Model->Group[Loop]->RF_Bitmap);
		}
