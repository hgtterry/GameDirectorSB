...
static geBoolean fdocBrushCSGCallback2 (const Brush *pBrush, void *lParam)
{
	...
	return 0;// (App->CL_Export_World->m_pDoc->BrushIsVisible(pBrush) && (!Brush_IsHint(pBrush)) && (!Brush_IsClip(pBrush)));
}
...
// changed QD 12/03 77
	...
	BList = App->CL_pDoc->pLevel->Brushes;

	if(!ExpSelected&&!ExpFiles)
	{
		fResult = App->CL_Export_World->Level_Build_G3ds(reinterpret_cast<tag_Level *> (App->CL_pDoc->pLevel), FileName, BList, ExpSelected, ExpLights, -1);
	}
	else
	{
		...
		for(i=0;i<GroupCount;i++)
		{
			...
			fResult = App->CL_Export_World->Level_Build_G3ds(reinterpret_cast<tag_Level *> (App->CL_pDoc->pLevel), NewFileName, SBList, ExpSelected, ExpLights, GroupID);
			if(!fResult)
...
// *************************************************************************
bool A_Export_World::Level_Build_G3ds(Level *pLevel, const char *Filename, BrushList *BList,int ExpSelected, geBoolean ExpLights, int GroupID)
{
	FILE	*f;
	geBoolean WriteRslt;
	int size, i;
	...
	// get the number of textures
		WrittenTex = (geBoolean*)calloc(sizeof(geBoolean), 0);// pLevel->WadFile->mBitmapCount);
