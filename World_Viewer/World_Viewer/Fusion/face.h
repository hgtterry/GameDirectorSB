...
void	Face_GetSplitInfo(const Face *f, const ::Plane *p, geFloat *dists, Ogre::uint8 *sides, Ogre::uint8 *cnt);
void	Face_Split(const Face	*f,		//original face
				   ...
				   geFloat		*dists,	//plane dists per vert
					Ogre::uint8		*sides);//plane sides per vert
void	Face_Clip(Face *f, const ::Plane *p, geFloat *dists, Ogre::uint8 *sides);
