...
WV_World::WV_World()
{
	...
	ItsAnOgreModel = 0;
	World_Loaded = 0;
	Model_Type = 0;// Enums::LoadedFile_None;
...
// *************************************************************************
void WV_World::Reset_Class(void)
{
	World_Loaded = 0;
	Model_Type = 0;// Enums::LoadedFile_None;
...
// *************************************************************************
void WV_World::Convert_To_GlobalMesh(void)
{
	int Count = 0;
	...
	App->CL_Scene->vertex_Data.resize(App->CL_Scene->VerticeCount);
	App->CL_Scene->Face_Data.resize(App->CL_Scene->FaceCount);

	App->CL_Scene->Normal_Data.resize(App->CL_Scene->VerticeCount);
	App->CL_Scene->MapCord_Data.resize(App->CL_Scene->VerticeCount);
	App->CL_Scene->MatIndex_Data.resize(App->CL_Scene->VerticeCount);
	...
	while (Count < GroupCount)
	{
		...
		while (GroupVertCount < App->CL_Scene->Group[Count]->GroupVertCount)
		{
			App->CL_Scene->vertex_Data[VC].x = App->CL_Scene->Group[Count]->vertex_Data[GroupVertCount].x;
			App->CL_Scene->vertex_Data[VC].y = App->CL_Scene->Group[Count]->vertex_Data[GroupVertCount].y;
			App->CL_Scene->vertex_Data[VC].z = App->CL_Scene->Group[Count]->vertex_Data[GroupVertCount].z;

			App->CL_Scene->MapCord_Data[VC].u = App->CL_Scene->Group[Count]->MapCord_Data[GroupVertCount].u;
			App->CL_Scene->MapCord_Data[VC].v = App->CL_Scene->Group[Count]->MapCord_Data[GroupVertCount].v;

			App->CL_Scene->Normal_Data[VC].x = App->CL_Scene->Group[Count]->Normal_Data[GroupVertCount].x;
			App->CL_Scene->Normal_Data[VC].y = App->CL_Scene->Group[Count]->Normal_Data[GroupVertCount].y;
			App->CL_Scene->Normal_Data[VC].z = App->CL_Scene->Group[Count]->Normal_Data[GroupVertCount].z;
	...
	{
		mFaceCount = 0;
		while (mFaceCount < App->CL_Scene->Group[Count]->GroupFaceCount)
		{

			App->CL_Scene->Face_Data[VC].Group = Count;

			App->CL_Scene->Face_Data[VC].a = App->CL_Scene->Group[Count]->Face_Data[mFaceCount].a + Offset;
			App->CL_Scene->Face_Data[VC].b = App->CL_Scene->Group[Count]->Face_Data[mFaceCount].b + Offset;
			App->CL_Scene->Face_Data[VC].c = App->CL_Scene->Group[Count]->Face_Data[mFaceCount].c + Offset;
		...
		Offset = Offset + App->CL_Scene->Group[Count]->GroupVertCount;
		Count++;
	...
	while (Count < GroupCount)
	{
		...
		while (GroupFaceCount < App->CL_Scene->Group[Count]->GroupFaceCount)
		{
			App->CL_Scene->MatIndex_Data[VC] = App->CL_Scene->Group[Count]->MaterialIndex;
...
// *************************************************************************
void WV_World::Set_Groupt_Count(int Count)
{
	GroupCount = Count;
}
...
// *		Set_Groupt_Count:- Terry and Hazel Flanigan 2023	  		   *
// *************************************************************************
void WV_World::Set_Texture_Count(int Count)
{
	TextureCount = Count;
}
...
// *			Set_Groupt_Count:- Terry and Hazel Flanigan 2023	   	   *
// *************************************************************************
void WV_World::Set_Motion_Count(int Count)
{
	MotionCount = Count;
}
...
// *************************************************************************
void WV_World::Set_Paths(void)
{
	strcpy(FileName, App->CL_Loader->FileName);
	strcpy(Path_FileName, App->CL_Loader->Path_FileName);
	...
	if (_stricmp(FileName + strlen(FileName) - 5, ".Wepf") == 0)
	{
		...
		GetFileTitleA(App->CL_Loader->Path_FileName, JustFileName, MAX_PATH);
...
// *************************************************************************
void WV_World::Create_Mesh_Group(int Index)
{
...
// *************************************************************************
void WV_World::Set_BondingBox_Model(bool Create)
{
	if (Create == 1)
	{
...
// *************************************************************************
// *					Clear Model and Reset Terry Flanigan			   *
// *************************************************************************
void WV_World::Clear_Model_And_Reset(void)
{
