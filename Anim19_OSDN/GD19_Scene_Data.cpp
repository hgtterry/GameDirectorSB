...
bool GD19_Scene_Data::Start_Scene()
{
	...
	Cl_Object[Index]->OgreEntity = App->Cl19_Ogre->mSceneMgr->createEntity("Start_Room","RF_Level1.mesh",App->Cl19_Ogre->PermResourceGroup);
	...
	App->Cl_Objects_New->Add_Stock_Message();
	//App->CL10_Objects_New->Add_Stock_Sound();
	App->Cl_Objects_New->Add_Stock_Panel();
...
LRESULT CALLBACK GD19_Scene_Data::Dialog_GetUserFile_Proc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		{
			bool test = App->Cl_Scene_Data->Open_Project_Dlg("Scene File   *.GDScene\0*.GDScene\0", "Scene File", NULL);
			if (test == 0)
			{
				return 0;
			}
