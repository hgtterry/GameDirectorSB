...
// *************************************************************************
LRESULT CALLBACK EB_Groups::Groups_Proc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
	case WM_INITDIALOG:
	...
	case WM_COMMAND:
	{
		...
		if (LOWORD(wParam) == IDC_CHANGETEXTURE)
		{
			if (App->CL_Vm_Model->Model_Loaded == 1)
			{
				App->EBC_Groups->ChangeTexture_ModelLocation();
			}
			return TRUE;
		}
		//// ------------------------------------------------- Show All Groups
...
// *************************************************************************
bool CALLBACK EB_Groups::ViewerBasePic(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	if (msg == WM_PAINT)
	{
		PAINTSTRUCT	ps;
		...
		if (App->EBC_Groups->Sel_BaseBitmap != NULL)
		{
			RECT	Source;
			RECT	Dest;
			HDC		hDC;
			...
			Source.left = 0;
			Source.top = 0;
			Source.bottom = App->EBC_Groups->BasePicHeight;
			Source.right = App->EBC_Groups->BasePicWidth;
			...
			App->EBC_Groups->RenderTexture_Blit(hDC, App->EBC_Groups->Sel_BaseBitmap, &Source, &Dest);
			ReleaseDC(hwnd, hDC);
		}
		EndPaint(hwnd, &ps);
		return 0;
...
// *************************************************************************
bool EB_Groups::RenderTexture_Blit(HDC hDC, HBITMAP Bmp, const RECT *SourceRect, const RECT *DestRect)
{
	HDC		MemDC;
	int		SourceWidth;
	int		SourceHeight;
...
// *************************************************************************
void EB_Groups::Update_Groups_Dialog(int Index)
{
	SelectedGroup = Index;
...
// *************************************************************************
bool EB_Groups::ChangeTexture_ModelLocation(void)
{
	...
	if (App->CL_Vm_Model->Model_Type == LoadedFile_Actor)
	{
		...
		App->CL_Vm_Textures->Soil_Load_Texture(App->CL_Vm_Textures->g_Texture, App->CL_Vm_FileIO->Texture_Path_FileName, App->CL_Vm_Model->S_TextureInfo[App->EBC_Groups->SelectedGroup]->ActorMaterialIndex);//App->S_MeshGroup[mIndex]->Soil_TextureIndex);
