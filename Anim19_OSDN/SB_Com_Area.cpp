...
void SB_Com_Area::Reset_Class(void)
{
	...
	while (Count < App->SBC_Scene->Area_Count) // Remove Ogre Objects
	{
		if (App->SBC_Scene->B_Area[Count])
		{
			...
			{
				App->CL_Ogre->mSceneMgr->destroySceneNode(App->SBC_Scene->B_Area[Count]->Area_Node);
				App->CL_Ogre->mSceneMgr->destroyEntity(App->SBC_Scene->B_Area[Count]->Area_Ent);
...
void SB_Com_Area::Add_Aera_To_Project(int Index, char* FileName, char* Resource_Location)
{
	...
	Area->Area_Ent = App->CL_Ogre->mSceneMgr->createEntity(App->SBC_Scene->B_Area[Index]->Area_Name, Mesh_File, App->SBC_Scene->Project_Resource_Group);
